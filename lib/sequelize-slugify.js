...
var SLUG_PLACEHOLDER = '__SEQUELIZE_SLUGIFY_PLACEHOLDER__';
...
SequelizeSlugify.prototype.slugifyModel = function (Model, slugOptions) {
     ...
     */
    var checkSlug = function (slug, transaction) {
        return Model.find({
            where: {
                slug: slug
            },
     ...
     */
    var addSourceSuffix = function (instance, sourceFields, suffixFields, transaction) {
        return (function suffixHelper(instance, sourceFields, suffixFields, suffixCount) {
            ...
            return checkSlug(slug, transaction).then(function (isUnique) {
                if (isUnique) {
                    return slug;
                }
     ...
     */
    var addNumericSuffix = function (slugValue, transaction) {
        return (function suffixHelper(slug, count) {
            var suffixedSlug = slug + '-' + count;

            return checkSlug(suffixedSlug, transaction).then(function (isUnique) {
                if (isUnique) {
                    return suffixedSlug;
                }
    ...
    var handleSlugify = function (instance, options, next) {
        ...
        // if we had no slug OR our slug value is the placeholder OR our slug changed and overwrite options is true build a new slug
        if (!slugValue || slugValue === SLUG_PLACEHOLDER || (slugOptions.overwrite && changed)) {
        ...
        return checkSlug(slugValue, options.transaction).then(function (isUnique) {
            ...
            return addSourceSuffix(instance, slugOptions.source, slugOptions.suffixSource, options.transaction);
        }).then(function (slug) {
            ...
            return checkSlug(slug, options.transaction);
            ...
            return addNumericSuffix(slugValue, options.transaction);
        }).then(function (slug) {
            // update the slug
            instance.slug = slug;
    ...
    var handleValidateOnCreate = function (instance, options) {
      // Bypass `allowNull: false` on creation, since we are going to add
      // the slug shortly hereafter
      if (instance.isNewRecord && !instance.slug) instance.slug = SLUG_PLACEHOLDER;
    };

    // attach model callbacks
    Model.beforeValidate('handleSlugify', handleValidateOnCreate);
