...
class SequelizeSlugify {
  slugifyModel(model, options) {
    ...
    const handleSlugify = async function (instance, sequelizeOptions, forceGenerateSlug) {
      ...
      // has a custom slug override been used
      if(slugifyOptions.slugOverride) {
        // check for a value in the override column
        const overrideSlugValue = instance[slugifyOptions.slugOverride];
        if(overrideSlugValue) {
          // set regular slug column to match
          instance[slugColumn] = overrideSlugValue;
          // return here to bypass the rest of the logic
          return instance;
        }
      }
